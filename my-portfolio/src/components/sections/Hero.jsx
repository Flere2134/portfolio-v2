    ...
    >
      ...
      <div className="relative z-10 mx-auto grid max-w-6xl grid-cols-1 gap-10 px-6 py-24 md:grid-cols-2 md:gap-14 md:px-10 md:py-32">
        ...
        <div className="flex flex-col justify-center">
          <span className="mb-6 inline-flex w-fit items-center rounded-full border border-teal/25 px-4 py-1.5 text-xs font-semibold tracking-[0.2em] text-teal dark:border-dark-brand/30 dark:text-dark-brand">
            {profile.badge}
          </span>

          <h1 className="text-4xl font-bold leading-[1.1] tracking-tight text-teal md:text-5xl dark:text-dark-brand">
            {profile.name},{" "}
            <span className="text-cerulean">{profile.title}</span>
          </h1>

          <p className="mt-6 max-w-xl text-base leading-relaxed text-espresso/80 md:text-lg dark:text-surface">
            {profile.bio}
          </p>
          ...
          <div className="mt-8 flex flex-col gap-4">
            {profile.cards.map((card) => (
              ...
              >
                ...
                <p className="mt-1.5 text-sm leading-relaxed text-espresso/75 dark:text-surface">
                  {card.body}
                </p>
              </div>
            ))}
        ...
        <div className="flex items-center justify-center md:justify-end">
          <div
            className="group relative w-full max-w-sm overflow-hidden rounded-3xl border border-espresso/10 bg-espresso p-8 transition-colors duration-300 hover:border-amber/30 dark:border-dark-surface dark:bg-dark-surface dark:hover:border-dark-accent/40"
